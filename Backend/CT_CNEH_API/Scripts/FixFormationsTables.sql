-- Script de correction pour le module Formation
-- Supprime et recrée les tables avec la bonne structure

USE CT_CNEH_DB;
GO

PRINT '=== CORRECTION DU MODULE FORMATION ===';
PRINT '';

-- 1. Supprimer les tables existantes si elles ont une mauvaise structure
PRINT '1. Vérification et suppression des tables problématiques...';
...
BEGIN
    ...
    BEGIN
        PRINT '   Suppression de la table Formations (structure incorrecte)...';
        DROP TABLE Formations;
        PRINT '   ✓ Table Formations supprimée';
    END
    ELSE
    BEGIN
        PRINT '   ✓ Table Formations a la bonne structure';
    END
END
...
BEGIN
    -- Vérifier si la table a la bonne structure
    IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'TypesFormation' AND COLUMN_NAME = 'Libelle')
    BEGIN
        PRINT '   Suppression de la table TypesFormation (structure incorrecte)...';
        DROP TABLE TypesFormation;
        PRINT '   ✓ Table TypesFormation supprimée';
    END
    ELSE
    BEGIN
        PRINT '   ✓ Table TypesFormation a la bonne structure';
    END
END

PRINT '';

-- 2. Créer la table TypesFormation si elle n'existe pas
IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'TypesFormation')
BEGIN
    PRINT '2. Création de la table TypesFormation...';
...
ELSE
BEGIN
    PRINT '2. ✓ Table TypesFormation existe déjà';
END

-- 3. Créer la table Formations si elle n'existe pas
IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'Formations')
BEGIN
    PRINT '3. Création de la table Formations...';
...
ELSE
BEGIN
    PRINT '3. ✓ Table Formations existe déjà';
END

-- 4. Vérifier que les tables de référence existent
PRINT '4. Vérification des tables de référence...';
